...
function toRouteOption(route: GoogleRoute, index: number, hasAccessibilityNeed: boolean): RouteOption {
...
export default defineEventHandler(async (event): Promise<RouteOption[]> => {
  ...
  const hasCoordinates = query.originLat !== undefined && query.originLng !== undefined
    && Number.isFinite(lat) && Number.isFinite(lng)
  const fallbackOrigin = String(config.googleRoutesOrigin || '').trim()
  if (!hasCoordinates && !fallbackOrigin) {
    throw createError({ statusCode: 400, statusMessage: '無法取得起點，請允許定位或設定 GOOGLE_ROUTES_ORIGIN' })
  }
  ...
  try {
    const response = await $fetch<GoogleRoutesResponse>('https://routes.googleapis.com/directions/v2:computeRoutes', {
      method: 'POST',
      headers: {
    ...
    return routes.map((route, index) => toRouteOption(route, index, /wheelchair|mobility/.test(needs)))
  } catch (error: any) {
